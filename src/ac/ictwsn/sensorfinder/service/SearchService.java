...
public class SearchService {
	 ...
	 */
	private Double computeDMRScore(String queryStr, int topnum){
		...
		Double[] queryBow = new Double[ms.getDictionary().size()];
		HashMap<String, Integer> wordDict = new HashMap<String, Integer>();
		for(String q : qList)
			if(ms.getDictionary().contains(q))
				wordDict.put(q, wordDict.get(q) + 1);
		double total = 0.0;
		for(int i=0; i<ms.getDictionary().size(); i++){
			if(wordDict.containsKey(ms.getDictionary().get(i))){
				queryBow[i] = (double)wordDict.get(ms.getDictionary().get(i));
				total += queryBow[i];
			}
		}
		for(int i=0; i<ms.getDictionary().size(); i++)
		...
		// compute topic vector of query
		//TODO compute phi (distribution of word)
		
		// compute vector similarity
		HashMap<String, Double> rankingMap = new HashMap<String, Double>();
		for(Entry<String, Double[]> entry : topicMap.entrySet()){
